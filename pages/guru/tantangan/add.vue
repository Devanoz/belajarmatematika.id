...
<div class="flex flex-col h-full px-4 py-4">
    <div class="title h-16 grid grid-cols-4 items-center">
        ...
        <h1 class="text-[1.4em] col-span-2 text-gray-700 font-bold text-center">Tambah Tantangan</h1>
    ...
    <div class="input-video">
        ...
        <form @submit.prevent="storeChallenge">
            <div class="materi-section mt-6 px-4 h-auto">
                ...
                <input id="judul" type="text" :disabled="challenge.id !=''" class="w-full text-gray-500 h-11 rounded-2xl border-2 border-gray-400 px-5 disabled:text-gray-400 " v-model="challenge.judul" />
            ...
            <div class="materi-section px-4 mt-4">
                ...
                <multiselect :disabled="challenge.id !=''" v-model="challenge.materi" :options="materis" :searchable="true" placeholder="Materi" selected-label="" select-label="" deselect-label="" track-by="title" label="title">
        ...
        <div v-if="challenge.id" class="make-question">
            <form @submit.prevent="storeQuestion">
                <ul class="nav nav-tabs grid grid-cols-4 mx-4 mt-6 list-none border-b-0 mb-4 px-2 bg-slate-400 rounded-2xl" id="tabs-tab" role="tablist">
                    <li class="nav-item col-span-2 text-center grid" role="presentation">
                        <button href="#tabs-home" type="button" @click="isPilihanGanda = true" :class="{'bg-gray-100 rounded-2xl': isPilihanGanda}" class="
      nav-link
      block
      justify-self-center
      font-medium
      ...
      active
    " id="tabs-home-tab" data-bs-toggle="pill" data-bs-target="#tabs-home" role="tab" aria-controls="tabs-home" aria-selected="true">Pilihan Ganda</button>
                    </li>
                    <li class="nav-item col-span-2 grid" role="presentation">
                        <button type="button" @click="isPilihanGanda = false" href="#tabs-profile" :class="{'bg-gray-100 rounded-2xl': !isPilihanGanda}" class="
      nav-link
      block
      text-xs
      mr-2
    ...
    " id="tabs-profile-tab" data-bs-toggle="pill" data-bs-target="#tabs-profile" role="tab" aria-controls="tabs-profile" aria-selected="false">
                ...
                <div class="tab-content px-4" id="tabs-tabContent">
                    <div v-if="isPilihanGanda == true" class="tab-pane fade show active" id="tabs-home" role="tabpanel" aria-labelledby="tabs-home-tab">
                        ...
                        <input id="judul" type="text" class="w-full h-11 mb-2 rounded-2xl border-2 text-gray-500 border-gray-400 px-5" v-model="question.options[0].body" />
                        <label class="mt-6 font-bold text-gray-500 mb-2">Pilihan B</label><br />
                        <input id="judul" type="text" class="w-full h-11 mb-2 rounded-2xl border-2 text-gray-500 border-gray-400 px-5" v-model="question.options[1].body" />
                        <label class="mt-6 font-bold text-gray-500 mb-2">Pilihan C</label><br />
                        <input id="judul" type="text" class="w-full h-11 mb-2 rounded-2xl border-2 text-gray-500 border-gray-400 px-5" v-model="question.options[2].body" />
                        <label class="mt-6 font-bold text-gray-500 mb-2">Pilihan D</label><br />
                        <input id="judul" type="text" class="w-full h-11 mb-2 rounded-2xl border-2 text-gray-500 border-gray-400 px-5" v-model="question.options[3].body" />

                        <label class=" font-bold text-gray-500 mb-2">Kunci Jawaban</label><br />
                        <multiselect v-model="question.answerKey" :options="question.options" :searchable="true" placeholder="Kunci" selected-label="" select-label="" deselect-label="" track-by="body" label="body">

                        </multiselect>

                        <div v-if="question.options[0].body !='' & question.options[1].body !='' & question.options[2].body !='' & question.options[3].body !=''" class="button-tambahkan flex flex-row justify-center">
                            <button type="submit" class="bg-orange-400 w-3/4 text-white font-bold py-2 mt-7 px-4 rounded-xl">
                                Tambahkan
                            </button>
                        </div>
...
export default {
    ...
    methods: {
        ...
        handleFileChange(e) {
            ...
            //get image
            let image = this.question.file = e.target.files[0]
            //check fileType
