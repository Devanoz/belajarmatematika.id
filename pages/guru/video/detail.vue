<template>
  <div id="root" class="flex flex-col h-full relative">
      ...
      <div :key="comment.id" id="comment-container" v-for="(comment,comment_idx) in comments" :id="comment.id">
        ...
        >
          <div class="flex">
            ...
            <div v-if="(comment.teacher_id)?comment.teacher_id === getTeacherId && show[comment_idx]:false"
                 id="handle-change"
                 class="absolute top-2 right-2 gap-2 flex flex-row justify-center items-center w-auto h-7 card-image drop-shadow-lg bg-gray-200 border-b-2 z-10 py-1 px-2 rounded-xl"
            >
          ...
          <div class="flex gap-5 mt-2">
            ...
            <transition name="fade">
              <div v-if="showBalas[comment_idx]" @click="onBalas(comment_idx)"
                   class="bg-blue-200 px-4 rounded-xl text-base"
              >Balas
              </div>
            </transition>
          </div>
        ...
        <!--        reply comment input-->
        <transition name="fade">
          <div v-if="showReplyComment[comment_idx]"
               class="bg-cs-blue-300 w-full p-4 pt-4 flex flex-row gap-4 justify-between"
          >
            <label class="w-full">
              <input v-model="studentReplyComment" placeholder="Tambah Comment" class="w-full py-3 px-4 rounded-full"
                     type="text"
              >
            </label>
            <div class="flex flex-col justify-center">
              <div @click="onReplyCommentSendCanceled(comment_idx)"
                   class="bg-yellow-300 h-10 w-auto px-2 rounded-xl font-bold flex flex-col justify-center items-center text-white"
              ><span>Cancel</span></div>
            </div>
            <div @click="onReplyCommentSend(comment.id,comment_idx)" id="send-button"
            ...
            >
              <svg width="35" height="35" viewBox="0 0 22 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                  ...
                  stroke="#56739D" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                />
              </svg>
            </div>
          </div>
          ...
          <div v-if="replyShow[comment_idx]">
            ...
            >
              ...
              <div v-if="comment.teacher || comment.student" id="student-detail" class="ml-3">
                ...
                >{{ reply.title }}</textarea>
              </div>
              <div v-if="reply.teacher_id === getTeacherId && replyShowList[comment_idx][reply_index]"
                   id="handle-change"
                   class="absolute top-2 right-2 gap-2 flex flex-row justify-center items-center w-auto h-7 card-image drop-shadow-lg bg-gray-200 border-b-2 z-10 py-1 px-2 rounded-xl"
              >
    ...
    <div v-if="showCommentSend && triggerComment"
         class="bg-cs-blue-300 w-full p-4 pt-4 flex flex-row gap-4 justify-between fixed bottom-0 z-30"
    >
      <label class="w-full">
        <input v-model="studentComment" placeholder="Tambah Comment" class="w-full py-3 px-4 rounded-full" type="text">
      </label>
      <div class="flex flex-col justify-center">
        <div @click="onCommentSendCanceled"
             class="bg-yellow-300 h-10 w-auto px-2 rounded-xl font-bold flex flex-col justify-center items-center text-white"
        ><span>Cancel</span></div>
      </div>
    ...
    <div v-if="!showCommentSend && !triggerComment" @click="toggleCommentSend"
         class="w-full p-4 pt-4 flex flex-row gap-4 justify-between fixed bottom-0 z-30 justify-center items-center"
    >
  ...
  methods: {
    ...
    confirmReplyEdit (reply_id, comment_idx) {
      ...
      this.$axios.post(`/api/teacher/replyComments/${reply_id}`, formData).then((response) => {
        ...
        this.comments = response.data.data.comments
        this.show = Array.from({ length: this.comments.length }, () => true)
        this.replyShowList[comment_idx] = this.replyShowList[comment_idx].map(() => {
          return true
        })
        this.$forceUpdate()
      }).catch(() => {
